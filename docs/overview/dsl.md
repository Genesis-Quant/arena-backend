...
- `industry_l0` 至 `industry_l3`：查询开始前先读取每只股票最后一条历史行业事件作为种子，
  再按 `code`、`time` 前向填充；`0` 表示该时点行业未知；
...
`IndustryWorker` 使用 Tushare `index_member_all` 的 `in_date`、`out_date` 构造行业初始快照、
变更事件和失效事件。CoreData 不逐日重复保存相同分类；查询层通过前向填充还原任意交易日快照。

| factor | value 语义 |
|---|---|
| `industry_l0` | 项目 11 类行业：1 能源、2 材料、3 工业、4 可选消费、5 日常消费、6 医疗保健、7 金融、8 信息技术、9 电信服务、10 公用事业、11 房地产 |
| `industry_l1` | 申万一级行业代码去掉 `.SI` 后的整数，例如 `801780.SI` 保存为 `801780` |
| `industry_l2` | 申万二级行业代码去掉 `.SI` 后的整数 |
| `industry_l3` | 申万三级行业代码去掉 `.SI` 后的整数 |

四个字段均使用 `0` 表示未知。`industry_l0` 是项目自定义映射，不是申万官方层级；原始申万
一至三级分类代码仍分别保存在 `industry_l1` 至 `industry_l3`。Worker 每次从 CoreData 读取四个字段的
最近完整事件日并重放该日，不使用本地水位文件；供应商历史修订需要使用覆盖模式重新抓取对应区间。
