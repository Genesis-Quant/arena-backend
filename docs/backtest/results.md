...
`daily` 和 `snapshot` 共用这四张表，snapshot 不新增分钟净值输出。收益及波动等仍按日收益计算；
原价快照没有分红、送转或除权补偿，跨除权日价格跳变会进入净值。真实盘口数量有限，需特别核对
部分成交、期末挂单和费用，不能按日线合成模式的近似无限容量解释。
...
| `-3` | 日终或回测结束时未成交失效 | 是 | 不是成交，也不等于主动撤单 |
...
下方“费用”记录了当前 `setPosition` 初始费用漏出 `totalFee` / `totalPnl` 的插件异常；该场景不能直接
使用 `totalEquity = initialCash + totalPnl`，也不能据 `totalFee=0` 判断没有扣费。
...
当前股票模式实测，开启最低单笔费用时，完整成交的买入费用为
`max(5, 成交额 × commission)`，卖出为 `max(5, 成交额 × (commission + tax))`，不是佣金保底后再加税。
关闭最低费用时去掉 `max(5, ...)`。例如成交额 1,000、佣金 0.0003、税率 0.001，开启最低费的
买入/卖出均为 5，关闭后分别为 0.3/1.3；成交额 100,000 时分别为 30/130；税率 0 时两侧相同。
这不是部分成交事件的通用分摊公式，部分成交仍需按实际事件逐项核验。

**已确认的插件限制（2026-09-05 复测）**：`setPosition` 建立初始持仓会扣除初始费用，但当前部署
未将该费用计入 `daily_portfolios.totalFee` 和 `totalPnl`。以初始资金 100,000、100 股、成本 11.73
为例：佣金 0 且关闭最低费用时现金为 98,827；开启最低费用后为 98,822；佣金 1% 且关闭最低费用时
为 98,815.27。后两者的费用字段仍为 0，权益与 `initialCash + totalPnl` 分别相差 5 和 11.73。
该差额已从现金/权益扣除，不能再扣一次；也不能从 `onTrade` 推断出完整初始建仓费用事件。
Arena 保留插件四表的原值，不伪造成交或改写费用/PnL。使用该接口的报告必须单独披露初始建仓现金
变化并标记这一账务限制；上游修复后需重新回测和对账，不能视为普通成交费用已通过验收。
