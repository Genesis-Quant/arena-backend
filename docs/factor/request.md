...
OLS 使用 SVD 求解；含截距的设计矩阵秩也由 SVD 判定。有效样本数不大于有效秩，或残差标准差
不大于 `max(abs(y)) × max(样本数, 设计矩阵列数) × DOUBLE机器精度` 时，处理值和分组保持空值。
不能把饱和回归的浮点舍入误差再次标准化成有效因子。回归输入按证券代码排序；分组按因子值、代码
升序确定次序，避免相同数据仅换行顺序就改变结果。常数市值/常数因子的标准化无定义；MAD 为零时
不截断；组数超过有效股票数时可有空组，这些情况不使用补值替代。
...
`<factor>_group`。非空因子行的分组值必须是 `0..n_groups-1` 内的整数，NULL、小数及越界值都会明确
报错，并返回列名、违规行数和日期/代码样例；不会截断小数或只在部分结果中丢弃非法组号。
...
极端组合按“因子值、证券代码”构成确定次序：bottom 两项升序，top 两项降序。并列因子不会依赖表的
物理行顺序；当股票足够时，全值并列也会从相反两端选取。`n_select` 超过半数时两端可能重叠。
市值权重不允许负数。权重或收益为 NULL 的行不参与该次加权，零权重不贡献收益；有效总权重非正时
返回 NULL。加权对数收益是 `sum(w × log_return) / sum(w)`，其指数不等于持有这些股票的市值加权财富；
验证实际投资组合回报应使用简单收益标签，不能混用这两种经济含义。
...
交易期按正式输出区间、原始查询结果中去重排序的日期推进，不按每组非空日期推进。中途空组或整个
截面被过滤不会缩短持有期：当前组为空时换手率为 NULL；有历史日期但历史组为空、当前组非空时为 1。
直接调用 `factorGroupTurnover` 可用末尾 `tradeDates` 参数传入完整日期轴；省略时使用传入表的全部
日期，不能补回已从该表删除的日期。Runtime 因子任务会显式传入过滤前的日期轴。
